...
- `assets/site.js` swaps in a placeholder for any image that has not been added yet, and
  runs the magnifying-glass search in the navigation bar.
- `assets/search-index.js` is the search index. It is regenerated by `build_pages.py`. If
  you edit page text by hand and want search to find the new wording, either rerun that
  script or edit the index file directly — the site works fine either way.
- Colours, type sizes and column widths were measured off the departmental faculty-page
  layout, so the home page matches it: dark bar at `#1c1c1c`, banner blue `#105d8f`,
  link red `#cc4331`, 15px justified body text on a 24px line, a 742px text column and a
  240px photo column.
